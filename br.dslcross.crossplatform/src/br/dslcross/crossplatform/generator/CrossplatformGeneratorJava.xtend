...
class CrossplatformGeneratorJava extends AbstractGenerator {
	...
	def treatVariableName(String variable){
		if(variable == "int"){
			println(" teste 1 : " + variable)
			return "Integer"
		}else if (variable == "void"){
			println(" teste 2 : " + variable)
			return variable.toFirstLower
		}else{
			println(" teste 3 : " + variable)
...
*/
    ...
    def compileType(DSLTypeRef typeRef) {
    	switch typeRef {
    		...
    		DSLConcreteTypeRef:  return specificReturnVariableGlogal(typeRef.ref.name) 
    		DSLGenericTypeRef: return specificReturnVariableGlogal(typeRef.ref.name)
    	}
    }	
 ...
 */	
	...
	«IF cmd.arrayAllow.equals(0) && cmd.concreteTypes.size == 0»	
	     «IF !(cmd.type instanceof Entity)» 
	        «"   "»«IF cmd.type.fullyQualifiedName.toString == "string"»«cmd.type.fullyQualifiedName.toString.toFirstUpper» «ELSE»«cmd.type.fullyQualifiedName» «ENDIF»«cmd.name»;
	     «ELSE»
	        «"   "»«IF cmd.type.fullyQualifiedName.toString == "string"»«cmd.type.fullyQualifiedName.toString.toFirstUpper» «ELSE»«cmd.type.fullyQualifiedName» «ENDIF»«cmd.name» = new «cmd.type.fullyQualifiedName»();
	     «ENDIF»
	...
	«ELSEIF cmd.concreteTypes.size > 0»
	        «"   "»«cmd.type.name.toFirstUpper»<«countAttributeConcrete(cmd)»> «cmd.name»  = new ArrayList<«countAttributeConcrete(cmd)»>();
	«ENDIF»        
	'''	
	...
	def countAttributeConcrete(Attribute atr){		
		...
		for(c: 0..< atr.concreteTypes.size){
			...
			attribute += comma + atr.concreteTypes.get(c).compileConcreteTypes.toFirstUpper 
		}
		return attribute		
	}
	...
	def compileMethodCallCmd(MethodCallCmd cmd) '''
	  «"   "»«cmd.obj»«FOR h : 0 ..< cmd.call.size».«cmd.call.get(h)»«ENDFOR»(«FOR i : 0 ..< cmd.args.size»«cmd.args.get(i).name»«FOR vatr: cmd.args.get(i).VAtr2».«vatr.name»«ENDFOR»«IF cmd.args.size>i+1», «ENDIF»«ENDFOR»);
	'''	
	...
	def compileCallAttributeObject(CallAttributeObject cmd)'''
	  «"   "»«cmd.obj.name»«compileVector(cmd.obj.v)»«FOR i : 0..< cmd.call.size».«cmd.call.get(i).name»«compileVector(cmd.call.get(i).v)»«ENDFOR» = «cmd.e.compileExpr»;
	'''	
...
def compileVector(Vector v){
	...
	def dictionaryJava(String term){
		if (term == "size"){
			return "size()"
		}else{
			return term
...
return args	
	   ...
	   «FOR platformOp : plat.cmdList»
	    «IF platformOp instanceof OperationGlobal» «««Nome declarado na entidade e a implementação no modelo de plataformas: linha de baixo	     
	     «IF contGlobalCall.opGlobCall.name.toString == platformOp.opGlobal.name.toString»	       
	       ...
	       «ELSE»
	       public «contGlobalCall.typeGlobal.compileType»«IF contGlobalCall.genericTypesDecl !== null»<«contGlobalCall.genericTypesDecl.name»>«ENDIF»«IF !(platformOp.col1 === null)»[]«ENDIF» «platformOp.opGlobal.name.toString»(«FOR i : 0 ..< contGlobalCall.params.size»«contGlobalCall.params.get(i).type.compileType + " " + contGlobalCall.params.get(i).name»«{countComma = 1 ""}»«IF (i+1)<contGlobalCall.params.size», «ENDIF»«ENDFOR»«FOR z : 0 ..< platformOp.params.size»«IF platformOp.params.get(z).type instanceof DSLConcreteTypeRef»«{verif=0 ""}»«FOR y : 0 ..< contGlobalCall.params.size»«IF platformOp.params.get(z).name.toString.equals(contGlobalCall.params.get(y).name)»«{verif = 1 ""}»«ENDIF»«ENDFOR»«IF verif != 1»«IF countComma > 0 && z >= 1», «ENDIF»«IF platformOp.params.get(z).ref !== null»«platformOp.params.get(z).type.compileType.toFirstUpper»<«platformOp.params.get(z).ref.compileConcreteTypes»> «platformOp.params.get(z).name.toString»«ELSE»«platformOp.params.get(z).type.compileType» «platformOp.params.get(z).name.toString»«ENDIF»«IF z+1<platformOp.params.size», «ENDIF»«ENDIF»«ENDIF»«ENDFOR»){
	       «ENDIF»
		   ...
		   if(EntityPlatform.toString.equals(en.name)){
		   	    ...
		   	    for(contGlobCall: en.features){
		   	    	if (contGlobCall instanceof OperationGlobCall){
		   	    		fsa.generateFile(e.fullyQualifiedName.toString("/") + compileConcreteTypes(contGlobCall.concreteTypes.get(0)) + "Fragment.java",
	...
	«IF f.arrayAllow.equals(0) && f.concreteTypes.size == 0»
		«" "»private «specificReturnVariable(f)» «f.name»;		
	...
	«" "»public «specificReturnVariable(f)»«IF !f.arrayAllow.equals(0)»[]«ENDIF» get«f.name.toFirstUpper»() {
	«" "»«" "»return «f.name»;
	«" "»}
	
	«" "»public void set«f.name.toFirstUpper»(«specificReturnVariable(f)»«IF !f.arrayAllow.equals(0)»[]«ENDIF» «f.name») {
	«" "»«" "»this.«f.name» = «f.name»;
	«" "»}
	«ENDIF»
	'''
	...
	def specificReturnVariable(Attribute f){
		println(" f.type.fullyQualifiedName :" + f.type.fullyQualifiedName)
		if (f.type.fullyQualifiedName.toString.toFirstUpper.toString == "Date" || f.type.fullyQualifiedName.toString.toFirstUpper.toString == "DateTime" || f.type.fullyQualifiedName.toString == "string") {
			return "String"
		}else if (f.type.fullyQualifiedName.toString == "bool" || f.type.fullyQualifiedName.toString == "Bool") {			
			return "Boolean"
		}else{			
			return f.type.fullyQualifiedName	
		}		
	}
	
	
	def specificReturnVariableGlogal(String f){
		
		if (f.toString.toFirstUpper.toString == "Date" || f.toString.toFirstUpper.toString == "DateTime" || f.toString == "string") {
			return "String"
		}else if (f.toString == "bool" || f.toString == "Bool") {			
			return "Boolean"
		}else{			
			return f	
		}		
