...
class CrossplatformGeneratorSwift extends AbstractGenerator {
	...
	def prepareParameters(Operation cF, int i, String varArray){
		...
		if (cF.params.get(i).ref === null){
			return cF.params.get(i).name + ": " + cF.params.get(i).type.compileType.toFirstUpper + varArray
		}else if (cF.params.get(i).ref !== null){
			return cF.params.get(i).name  + ": ["+cF.params.get(i).ref.name.toString.toFirstUpper+"]" + varArray 
		}else{
			return cF.params.get(i).name + ": " + cF.params.get(i).type.compileType + varArray
		}		
	}
	//featVar.params.get(i).name + ": " + featVar.params.get(i).type.compileType + varArray
		...
		for (e : resource.allContents.toIterable.filter(Entity)) { // Gerar a entidade de acordo com a EntityPlatform
			...
			if (EntityPlatform == e.name.toString) {
				...
				for (dep : resource.allContents.toIterable.filter(Deploy)) {
					for (pl : dep.entityToDeploy) {
						if ((pl.ent.name.toString == EntityPlatform) &&
							...
							println("\n Plataforma da entidade: iOS >>>>>>>>>>> " + PlataformaDaEntidade)
						}
					}
				}
				...
				if (verifController.equals(0)){ //Não é controller
					fsa.generateFile(e.fullyQualifiedName.toString("/") + ".swift",
	...
	class «e.name» «IF e.superType !== null» extends «e.superType.fullyQualifiedName» «ENDIF»{
			...
			«FOR i : 0 ..< countOperation»
				...
				«var cpClassOP = 0»
					...
					«IF	verifyOpEntName(op).equals(op.name)»		      
				      init(«FOR c1 : classOpSignParamArray2D.get(i).size >.. 0»«IF classOpSignParamArray2D.get(i).get(c1) !== null»«classOpSignParamArray2D.get(i).get(c1).toString»«IF c1 > 0», «ENDIF»«ENDIF»«ENDFOR») {
					««««NormalMethod
					«ELSE»
				      func «IF op.genericTypesDecl.size > 0»«ENDIF»«IF !(op.col1 === null)»[]«ENDIF» «op.name»(«FOR c1 : 0 ..< classOpSignParamArray2D.get(i).size»«IF classOpSignParamArray2D.get(i).get(c1) !== null»«classOpSignParamArray2D.get(i).get(c1).toString»«IF c1+1 < cpClassOP», «ENDIF»«ENDIF»«ENDFOR») «IF op.type.compileType !== null» -> «prepareParametersReturn(op)»«ENDIF» {
					«ENDIF»
	...
	def prepareParametersReturn(Operation op){
		println(" swift retorno: " + " " + op.genericTypesDecl.size)
 ...
 */	
	...
	«IF cmd.arrayAllow.equals(0) && cmd.concreteTypes.size == 0»	 
	     «IF !(cmd.type instanceof Entity)» 
	        «"   "»var «cmd.name»: «IF cmd.type instanceof DataType»«cmd.type.fullyQualifiedName.toString.toFirstUpper»«ELSE»«cmd.type.fullyQualifiedName»«ENDIF»«IF cmd.type instanceof Entity»()«ENDIF»««««««In case of cmd be a class or entity it´s put a ()
	     «ELSE»
	        «"   "»let «cmd.name» =  «IF cmd.type instanceof DataType»«cmd.type.fullyQualifiedName.toString.toFirstUpper»«ELSE»«cmd.type.fullyQualifiedName»«ENDIF»«IF cmd.type instanceof Entity»()«ENDIF»««««««In case of cmd be a class or entity it´s put a ()
	     «ENDIF»
	«ELSEIF !cmd.arrayAllow.equals(0) && cmd.concreteTypes.size == 0»
	        «"   "»«cmd.name» = [«IF cmd.type instanceof DataType»«cmd.type.fullyQualifiedName.toString.toFirstUpper»«ELSE»«cmd.type.fullyQualifiedName»«ENDIF»]()
	        
	«ELSEIF cmd.concreteTypes.size > 0»
	        «"   "»«cmd.name» = [«countAttributeConcrete(cmd)»]()
	«ENDIF»        
	'''	
	...
	def countAttributeConcrete(Attribute atr){		
		...
		for(c: 0..< atr.concreteTypes.size){
			...
			attribute += comma + atr.concreteTypes.get(c).compileConcreteTypes 
		}
		return attribute.toFirstUpper //only for swift case		
	}
	...
	def compileAbrevCmd(AbrevCmd cmd) '''«IF cmd.expr1 !== null»«IF cmd.opIn == "++"»«cmd.expr1.name»+=1«ELSEIF cmd.opIn == "--"»«cmd.expr1.name»-=1«ENDIF»«ENDIF»«IF cmd.expr2 !== null && cmd.expr3 !== null»«cmd.expr2.name»«cmd.opIn»«cmd.expr3.compileExpr»«ENDIF»«IF cmd instanceof AssignmentCmdFor»«cmd.left.name»=«cmd.e.compileExpr»«ENDIF»'''			
	    
	def compileMethodCallCmd(MethodCallCmd cmd) '''
	  «"   "»«cmd.obj»«FOR h : 0 ..< cmd.call.size».«cmd.call.get(h).adjustCommand»«ENDFOR»(«FOR i : 0 ..< cmd.args.size»«IF getParameterNameOperation(cmd, i) == ""»«cmd.args.get(i).name»«ELSE»«getParameterNameOperation(cmd, i)»«ENDIF»: «cmd.args.get(i).name»«FOR vatr: cmd.args.get(i).VAtr2».«vatr.name»«ENDFOR»«IF cmd.args.size>i+1», «ENDIF»«ENDFOR»)	  
	'''	
	
	def getParameterNameOperation(MethodCallCmd cmd, int i){ //get the param to generate the correct call of function, ex: func(name: name, price: price)		
		var variable = ""
		for (vatr: cmd.args.get(i).VAtr2){
			variable = vatr.name
		}
		return variable
	}
...
def compileVector(Vector v){
	...
	def compileOpLogicoFor(OpLogicoFor opCmd) ''' ««««« treating for as while in swift
	
«"   "»«opCmd.exprLogic.inicio.left.name»=«opCmd.exprLogic.inicio.e.compileExpr»
«"   "»while «opCmd.exprLogic.condicao.compileExpLogico()» {
«FOR cmd2 : opCmd.cmdList»
«"   "»«compileOperationCmd(cmd2)»
«ENDFOR»
«"   "»«opCmd.exprLogic.abrevCmd.compileOperationCmd»
«"   "»}
...
'''	
	...
	def compileExpLogicoSwift(ExprLogica exprLog) '''«FOR i:0..<exprLog.termos.size»«IF i>0» «exprLog.oplog.get(i-1).compileOpLog» «ENDIF»«exprLog.termos.get(i).compileTermoLogicoSwift()»«ENDFOR»'''
	def compileTermoLogicoSwift(TermoLogico termoLog) '''«FOR i:0..<termoLog.fatores.size»«IF i>0» «termoLog.oplog.get(i-1).compileOpLog» «ENDIF»«termoLog.fatores.get(i).compileFatorLogSwift()»«ENDFOR»'''
	def compileFatorLogSwift(FatorLogico fatorLog)'''«IF fatorLog.expr2.i !=0»«fatorLog.expr2.i»«IF fatorLog.expr2.ID != 0».«fatorLog.expr2.ID»«ENDIF»«ELSEIF fatorLog.expr2.strVar !== null»"«fatorLog.expr2.strVar»"«ELSEIF fatorLog.expr2.VAtr1 !== null»«fatorLog.expr2.VAtr1»«FOR i : 0 ..< fatorLog.expr2.VAtr2.size».«fatorLog.expr2.VAtr2.get(i).name»«ENDFOR»«ELSE»«IF fatorLog.expr2.v === null»«fatorLog.expr2.i»«IF fatorLog.expr2.ID != 0».«fatorLog.expr2.ID»«ENDIF»«ELSEIF fatorLog.expr2.strVar !== null»«fatorLog.expr2.strVar»«ELSE»«fatorLog.expr2.v»«ENDIF»«ENDIF»«IF fatorLog.opRel === null && fatorLog.op1 === null && fatorLog.op2 === null && fatorLog.v===null»(«compileExpLogico(fatorLog.exprLog)»)«ENDIF»'''	
	/*---------------Expressões lógicas para Swift para montagem do for-------------- */
...
return args	
	   ...
	   «FOR platformOp : plat.cmdList»
	    «IF platformOp instanceof OperationGlobal» «««Nome declarado na entidade e a implementação no modelo de plataformas: linha de baixo	     
	     «IF contGlobalCall.opGlobCall.name.toString == platformOp.opGlobal.name.toString»	       
	       ...
	       «contGlobalCall.typeGlobal.compileType»«IF !(platformOp.col1 === null)»[]«ENDIF» «contGlobalCall.concreteTypes.get(0).name»(«FOR i : 0 ..< contGlobalCall.params.size»«contGlobalCall.params.get(i).type.compileType + " " + contGlobalCall.params.get(i).name»«{countComma = 1 ""}»«IF (i+1)<contGlobalCall.params.size», «ENDIF»«ENDFOR»«FOR z : 0 ..< platformOp.params.size»«IF platformOp.params.get(z).type instanceof DSLConcreteTypeRef»«{verif=0 ""}»«FOR y : 0 ..< contGlobalCall.params.size»«IF platformOp.params.get(z).name.toString.equals(contGlobalCall.params.get(y).name)»«{verif = 1 ""}»«ENDIF»«ENDFOR»«IF verif != 1»«IF countComma > 0 && z >= 1», «ENDIF»«IF platformOp.params.get(z).ref !== null»«platformOp.params.get(z).type.compileType.toFirstUpper»<«platformOp.params.get(z).ref.compileConcreteTypes»> «platformOp.params.get(z).name.toString»«ELSE»«platformOp.params.get(z).type.compileType» «platformOp.params.get(z).name.toString»«ENDIF»«IF z+1<platformOp.params.size», «ENDIF»«ENDIF»«ENDIF»«ENDFOR») «IF contGlobalCall.typeGlobal.compileType !== null» -> «contGlobalCall.typeGlobal.compileType.toFirstUpper»«ENDIF»{ 
	       «ELSE»
	        func «platformOp.opGlobal.name.toString»(«FOR i : 0 ..< contGlobalCall.params.size»«contGlobalCall.params.get(i).name + ": " + prepareParametersReturnGlobal(contGlobalCall, i)»«{countComma = 1 ""}»«IF (i+1)<contGlobalCall.params.size», «ENDIF»«ENDFOR»«FOR z : 0 ..< platformOp.params.size»«IF platformOp.params.get(z).type instanceof DSLConcreteTypeRef»«{verif=0 ""}»«FOR y : 0 ..< contGlobalCall.params.size»«IF platformOp.params.get(z).name.toString.equals(contGlobalCall.params.get(y).name)»«{verif = 1 ""}»«ENDIF»«ENDFOR»«IF verif != 1»«IF countComma > 0 && z >= 1», «ENDIF»«IF platformOp.params.get(z).ref !== null»«platformOp.params.get(z).name.toString»: «prepareParametersReturnOperationGlobal(platformOp, z)»«ELSE»«platformOp.params.get(z).name.toString»: «platformOp.params.get(z).type.compileType»«ENDIF»«IF z+1<platformOp.params.size», «ENDIF»«ENDIF»«ENDIF»«ENDFOR») «IF contGlobalCall.typeGlobal.compileType !== null» -> «prepareTypeReturnGlobal(contGlobalCall)»«ENDIF»{
	        ««««««««« func «IF op.genericTypesDecl.size > 0»<«FOR c2 : 0 ..< op.genericTypesDecl.size»«op.genericTypesDecl.get(c2).name.toFirstUpper»«IF c2+1 < op.genericTypesDecl.size», «ENDIF»«ENDFOR»>«ENDIF»«IF !(op.col1 === null)»[]«ENDIF» «op.name»(«FOR c1 : classOpSignParamArray2D.get(i).size >.. 0»«IF classOpSignParamArray2D.get(i).get(c1) !== null»«classOpSignParamArray2D.get(i).get(c1).toString»«IF c1 > 0», «ENDIF»«ENDIF»«ENDFOR») «IF op.type.compileType !== null» -> «op.type.compileType.toFirstUpper»«ENDIF» {
	       «ENDIF»
	...
	def prepareTypeReturnGlobal(OperationGlobCall op){

		var listType = ""		
        if (op.genericTypesDecl !== null){
		  listType = op.genericTypesDecl.name
        }	
		if (listType === ""){
			return op.typeGlobal.compileType.toFirstUpper
		}else{
			return "["+listType.toFirstUpper+"]"
		}
	}
	
	
	def prepareParametersReturnGlobal(OperationGlobCall op, int i){		
		//contGlobalCall.params.get(i).type.compileType
		var listType = ""		
        if (op.params.get(i).ref !== null){
		     listType = op.params.get(i).ref.name
        }	
		if (listType === ""){
			return op.params.get(i).type.compileType
		}else{
			return "["+listType.toFirstUpper+"]"
		}
	}	
	...
	def prepareParametersReturnOperationGlobal(OperationGlobal op, int i){		
		//«platformOp.params.get(z).type.compileType.toFirstUpper»<«platformOp.params.get(z).ref.compileConcreteTypes»>
		var listType = ""		
        if (op.params.get(i).ref !== null){
		     listType = op.params.get(i).ref.compileConcreteTypes
        }	
		if (listType === ""){
			return op.params.get(i).type.compileType
		}else{
		   ...
		   if(EntityPlatform.toString.equals(en.name)){
		   	    ...
		   	    for(contGlobCall: en.features){
		   	    	if (contGlobCall instanceof OperationGlobCall){
		   	    		fsa.generateFile(e.fullyQualifiedName.toString("/") + compileConcreteTypes(contGlobCall.concreteTypes.get(0)) + ".swift",
	...
	class «e.name»«compileConcreteTypes(contGlobCall.concreteTypes.get(0))»Fragment «IF e.superType !== null» extends «e.superType.fullyQualifiedName» «ENDIF»{
	...
	«IF f.arrayAllow.equals(0) && f.concreteTypes.size == 0»
	    «" "»private var «f.name»: «specificReturnVariable(f)»«initializationPojo(f)»
	«ELSEIF !f.arrayAllow.equals(0) && f.concreteTypes.size == 0»		
		«" "»private var «f.name» = [«IF f.type instanceof DataType»«f.type.fullyQualifiedName.toString.toFirstUpper»«ELSE»«f.type.fullyQualifiedName»«ENDIF»]() 
	«ELSEIF f.concreteTypes.size > 0»
		 «" "»private var «f.name» = [«countAttributeConcrete(f)»]()
	«ENDIF»

    «IF f.concreteTypes.size == 0»
	«" "»public func  get«f.name.toFirstUpper»() «IF f.type.compileConcreteTypes !== null» -> «specificReturnGetterSetterMethod(f)»«ENDIF» { 
	«" "»«" "»return self.«f.name»
	«" "»}
	
	«" "»public func set«f.name.toFirstUpper»(«f.name»: «specificReturnGetterSetterMethod(f)») {
	«" "»«" "»self.«f.name» = «f.name»
	«" "»}
	«ENDIF»
	'''
	...
	def specificReturnVariable(Attribute f){
		if (f.type.fullyQualifiedName.toString.toFirstUpper == "Date" || f.type.fullyQualifiedName.toString.toFirstUpper == "DateTime") {
			return "String"
		}else{
			return f.type.fullyQualifiedName.toString.toFirstUpper
		}		
	}
	
	def specificReturnGetterSetterMethod(Attribute f){
		if (!f.arrayAllow.equals(0)) {
		  return "["+f.type.compileConcreteTypes.toFirstUpper+"]"
		}else{
		  if (f.type.fullyQualifiedName.toString.toFirstUpper == "Date" || f.type.fullyQualifiedName.toString.toFirstUpper == "DateTime") {
		  	return "String"
		  }else{
	...
	def initializationPojo(Attribute f)'''
	   «var type = f.type.fullyQualifiedName.toString.toFirstUpper»
	   «IF !(getType(f))»	    
	    «IF type == "string" || type == "String" || type == "Date" || type == "dateTime" || type == "DateTime"»
	     «" "»= ""
...
def getType(Attribute f){	//Verify if the Attribute is DataType or Entity to initialize in correct way in swift language
if(f.type instanceof Entity) {
   //var r1 = f.eContainer   
   return true		  							
}else if(f.type instanceof DataType){
  return false	
}else{
  return false
}	
	...
	def adjustCommand(String command){
		var adjustedCommand = ""		
		if (command == "add"){
